...
public class EmailUtils {
    ...
    public void sendEmail(String title, String messageBody,
        ...
        try {
            ...
            props.put("mail.smtp.host", InfraKafkaUtil.ALARM_SMTP_SERVER); // smtp主机名。
            props.put("mail.smtp.user", InfraKafkaUtil.ALARM_SMTP_USERNAME); // 发送方邮件地址。
            props.put("mail.smtp.password", InfraKafkaUtil.ALARM_SMTP_PASSWORD); // 邮件密码。
            PopupAuthenticator popA = new PopupAuthenticator();// 邮件安全认证。
            PasswordAuthentication pop =
                    popA.performCheck(InfraKafkaUtil.ALARM_SMTP_USERNAME,
                    		InfraKafkaUtil.ALARM_SMTP_PASSWORD); // 填写用户名及密码
            sendMailSession = Session.getInstance(props, popA);
            Message newMessage = new MimeMessage(sendMailSession);
            newMessage.setFrom(new InternetAddress(InfraKafkaUtil.ALARM_SMTP_USERNAME));
