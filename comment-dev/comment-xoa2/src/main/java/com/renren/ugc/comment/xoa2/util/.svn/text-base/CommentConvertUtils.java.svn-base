...
import com.renren.ugc.comment.xoa2.SimpleComment;
...
public class CommentConvertUtils {
     ...
     */
    public static com.renren.ugc.comment.xoa2.Comment toXoa2Comment(com.renren.ugc.comment.model.Comment c) {
        ...
        comment.setSimpleComment(toXoa2SimpleComment(c.getSimpleComment()));
        comment.setFloor(c.getFloor());
        comment.setCommentPhotoUrl(c.getCommentPhotoUrl());
        comment.setCommentPhotoId(c.getCommentPhotoId());
        return comment;
    }
    
    /**
     * 评论中心model转xoa2协议model
     * 
     * @param commentInfo
     * @return
     */
    public static SimpleComment toXoa2SimpleComment(SimpleCommentInfo commentInfo){
    	if(null == commentInfo){
    		return null;
    	}
    	SimpleComment simpleComment = new SimpleComment();
    	simpleComment.setAuthorId(commentInfo.getAuthorId());
    	simpleComment.setId(commentInfo.getId());
    	simpleComment.setAuthorName(commentInfo.getAuthorName());
    	simpleComment.setContent(commentInfo.getContent());
    	simpleComment.setFloor(commentInfo.getFloor());
    ...
    /**
     * convert a list of internal comment object to a list of interface comment.
     * This method assumes that the input list is not null if needMetadata is
     * true,set metadata to the Comment the filed of "params"
     */
    public static List<com.renren.ugc.comment.xoa2.Comment> toXoa2CommentList(
        List<com.renren.ugc.comment.model.Comment> cList, boolean needMetadata) {
        List<com.renren.ugc.comment.xoa2.Comment> commentList =
                new ArrayList<com.renren.ugc.comment.xoa2.Comment>(cList.size());
        for (com.renren.ugc.comment.model.Comment c : cList) {
            commentList.add(CommentConvertUtils.toXoa2Comment(c, needMetadata));
        }
    ...
    /**
     * convert a map of internal comment object to a list of interface comment.
     * This method assumes that the input map is not null
     */
    public static Map<Long, com.renren.ugc.comment.xoa2.Comment> toXoa2CommentMap(
        Map<Long, com.renren.ugc.comment.model.Comment> cMap, boolean needMetadata) {
        Map<Long, com.renren.ugc.comment.xoa2.Comment> commentMap =
                new HashMap<Long, com.renren.ugc.comment.xoa2.Comment>(
                    cMap.size());
        for (long id : cMap.keySet()) {
        	com.renren.ugc.comment.model.Comment c = cMap.get(id);
            commentMap.put(id,
                CommentConvertUtils.toXoa2Comment(c, needMetadata));
        }

        return commentMap;
    }

    public static com.renren.ugc.comment.xoa2.Entry toXoa2Entry(Entry e) {
        ...
        if (e != null) {
            ...
            entry.setEntryProps(e.getEntryProps());
